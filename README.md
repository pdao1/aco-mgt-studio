...
- Read-only Gmail IMAP polling of All Mail (with Inbox fallback). Discovery searches body/subject order, purchase, receipt, shipping, pickup, cancellation and refund keywords, plus mail from known retailer/carrier domains. These are candidates, not proof of an order; parsing still requires an order/tracking identifier and excludes one-time PIN mail.
- Header-first de-duplication applies before `SYNC_MAX_MESSAGES` (default 500). Each batch splits capacity between oldest outstanding mail and newest updates. Polling revisits the saved history window; manual **Sync** requests a durable 365-day backfill. Large inboxes are processed across multiple batches: subsequent polls continue the deep-scan window, even after a restart, until it drains. Manual Sync can also advance the next batch. Mail outside All Mail (such as Trash/Spam), older than the lookback, or without a supported identifier is not guaranteed to be imported.
- Processed mail is tagged with `MAILBOX_PARSER_VERSION`; migration 014 makes legacy messages eligible for a one-time replay under the new rules. Later Sync runs skip current-version decisions, not old parser decisions. AI repair deferred by budget exhaustion/provider failure stays eligible; optional item review does not block a deterministic order. Bump the version for future extraction-rule changes. Repair upserts existing orders/events and preserves archive/item-visibility choices.
- Removing an inbox deletes its mailbox credentials, orders, shipments, events, processed-message keys, and sync runs through customer-scoped cascades. Reconnecting creates a new customer ID and starts a fresh import within the selected lookback. Removal is not necessary for parser-version backfills.
...
- Order rows can be archived from active calculations, and individual item rows can be hidden/restored in the operator inspector. These corrections are recorded as redacted parser feedback, used as same-retailer review examples, and downloadable from Settings as JSONL for evaluation.
