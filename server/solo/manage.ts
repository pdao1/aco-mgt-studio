...
const {values}=parseArgs({options:{handle:{type:'string'},name:{type:'string'},'discord-id':{type:'string'},days:{type:'string',default:'30'},'mailbox-limit':{type:'string',default:'2'},rotate:{type:'boolean',default:false},renew:{type:'boolean',default:false}}});
...
try {
  ...
  if(values.renew) {
    ...
    const result=await accounts.provision({handle,displayName:z.string().trim().min(1).max(120).parse(values.name??handle),
      ...
      days,mailboxLimit:z.coerce.number().int().min(1).max(2).parse(values['mailbox-limit'])});
